...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, R(rd) = s->pc + 4;
          s->dnpc = (src1 + imm) & 0xfffffffe; ftrace_ret_func(s->pc, s->dnpc));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti, I,
          R(rd) =
              ((int32_t)src1 < (int32_t)imm) ? 1 : 0); // sext 确实是返回无符号
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R,
          R(rd) = ((int64_t)(int32_t)src1 * (uint64_t)src2) >> 32); // RV32

  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R,
          R(rd) = ((uint64_t)src1 * (uint64_t)src2) >> 32); // RV32
  // type j
  // 2.5.1. Unconditional Jumps
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4;
          s->dnpc = s->pc + imm; ftrace_call_func(s->pc, s->dnpc));
  ...
  INSTPAT(
      "??????? ????? ????? 000 ????? 11000 11", beq, B, if (src1 == src2) {
        s->dnpc = s->pc + imm;
        ftrace_call_func(s->pc, s->dnpc);
      });

  INSTPAT(
      "??????? ????? ????? 001 ????? 11000 11", bne, B, if (src1 != src2) {
        s->dnpc = s->pc + imm;
        ftrace_call_func(s->pc, s->dnpc);
      });
  ...
  INSTPAT(
      "??????? ????? ????? 111 ????? 11000 11", bgeu, B, if (src1 >= src2) {
        s->dnpc = s->pc + imm;
        ftrace_call_func(s->pc, s->dnpc);
      });
  // 2.5.2 Conditional Branche  // 13.2. Division Operations
  INSTPAT(
      "??????? ????? ????? 100 ????? 11000 11", blt, B,
      if ((int32_t)src1 < (int32_t)src2) {
        s->dnpc = s->pc + imm;
  ...
  INSTPAT(
      "??????? ????? ????? 110 ????? 11000 11", bltu, B, if (src1 < src2) {
        s->dnpc = s->pc + imm;
        ftrace_call_func(s->pc, s->dnpc);
      });
